...
public class ReceiverThread extends Thread {
    ...
    private long serverOutAtTimeMilis;
    private long serverOnAtTimeMilis;
    private long serverDownDurationMilis;

    public ReceiverThread(String host, int port, PacketScheduler scheduler) {
        this.host = host;
        this.port = port;
        this.packetScheduler = scheduler;
    }

    //  u prvoj petlji pravimo rekonekcije ka serveru u slucaju pucanja server ili gasenja soketa od strane servera
    // 
    @Override
    public void run() {
        while (true) {

            // pokusavamo da se konektujemo u loopu dok se konekcija ne osvari i merimo vreme
            ...
            try (socket;
                ...
                while (true) {
                    ...
                    try {
                        ...
                        // server je prekinuo sa radom
                        System.out.println("PUKLA JE VEZA");
                        serverOutAtTimeMilis = System.currentTimeMillis();
                        break; // exit read loop → reconnect
                    }
                }

            } catch (IOException e) {
                System.out.println("GRESKA NAKON USPOSTAVLJENE KONEKCIJE");
                e.printStackTrace();
            }
    ...
    // metoda za uspostavljanje konekcija i merenja downtime-a konekcija
        ...
        while (true) {
            try {
                ...
                if (downtimeStart == 0) {
                    ...
                    System.out.println("NE MOZE DA SE USPOSTAVI KONEKCIJA (starting downtime timer)");
                } else {
